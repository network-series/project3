    ...
    while True:
        ...
        if message.decode()[0:4] == "helo":
            ...
            if num == 0:
                ...
                in_file=open("D:\project\cn_program3\log.txt",'a')
                in_file.writelines(str(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))+"  "+seq+"  "+"deny")
                in_file.close()
            else:
                number = -1
                ...
                in_file=open("D:\project\cn_program3\log.txt",'a')
                in_file.writelines(str(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))+"  "+seq+"  "+"agree")
                in_file.close()
            ...
            if int(n) in range(1, df['num'][g_point] + 1):
                ...
                in_file=open("D:\project\cn_program3\log.txt",'a')
                in_file.writelines(str(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))+"  "+seq+"  "+"return agree")
                in_file.close()
            else:
                conn.sendto("rtrn deny".encode(), client_addr)
                in_file=open("D:\project\cn_program3\log.txt",'a')
                in_file.writelines(str(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))+"  "+seq+"  "+"return deny")
                in_file.close()
